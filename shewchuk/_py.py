...
class Expansion:
    ...
    def __new__(cls, *args, _compress=True):
        ...
        if len(args) == 1:
            ...
            if isinstance(argument, Expansion):
                ...
                components = [argument]
            elif isinstance(argument, Integral):
    ...
    def __ceil__(self):
        return (_components_to_integer(self._components)
                + ceil(_components_to_accumulated_fraction(
                        self._components)))
    ...
    def __floor__(self):
        return (_components_to_integer(self._components)
                + floor(_components_to_accumulated_fraction(
                        self._components)))
    ...
    def __rtruediv__(self, other):
        return (other * Expansion(*_invert_components(self._components))
                if isinstance(other, (Integral, float))
                else NotImplemented)
...
_EPSILON = float_info.epsilon / 2.


def _add_components_eliminating_zeros(left: Sequence[float],
...
def _are_components_equal_to_float(components: Sequence[float],
                                   value: float) -> bool:
    return len(components) == 1 and components[0] == value


def _are_components_lesser_than_float(components: Sequence[float],
                                      value: float) -> bool:
...
def _are_components_lesser_than_integral(components: Sequence[float],
...
def _are_components_lesser_than(left: Sequence[float],
...
def _add_float_eliminating_zeros(components: Sequence[float],
...
def _components_to_accumulated_fraction(components: Sequence[float]
                                        ) -> float:
    result = 0.
    for component in components:
...
def _components_to_fractions(components: Sequence[float]
                             ) -> Sequence[float]:
    result = []
    for component in components:
        component_fraction = _to_fraction(component)
...
def _compress_components(components: Sequence[float]) -> Sequence[float]:
    for _ in repeat(None, len(components)):
...
def _compress_components_single(components: Sequence[float]
                                ) -> Sequence[float]:
...
def _divide_components(dividend: Sequence[float],
                       divisor: Sequence[float]) -> Sequence[float]:
    return _multiply_components_eliminating_zeros(dividend,
                                                  _invert_components(divisor))


def _integral_to_components(value: Integral) -> Sequence[float]:
    if not value:
        return [0.]
    value = int(value)
    result = []
...
def _invert_components(components: Sequence[float]) -> Sequence[float]:
    ...
    for _ in repeat(None, 6 + _ceil_log2(len(components))):
...
def _is_float_lesser_than_components(value: float,
                                     components: Sequence[float]) -> bool:
    return value < components[-1] or (len(components) > 1
                                      and components[-1] == value
                                      and components[-2] > 0.)


def _is_integral_lesser_than_components(value: Integral,
...
def _multiply_components_eliminating_zeros(left: Sequence[float],
                                           right: Sequence[float]
                                           ) -> Sequence[float]:
    return reduce(_add_components_eliminating_zeros,
...
def _scale_components_eliminating_zeros(components: Sequence[float],
                                        scalar: float) -> Sequence[float]:
...
def _split(value: float,
           *,
           splitter: float
           = float(1 + (1 << ((float_info.mant_dig + 1) // 2)))
           ) -> Tuple[float, float]:
    base = splitter * value
...
def _subtract_components_eliminating_zeros(minuend: Sequence[float],
...
def _subtract_double_eliminating_zeros(minuend: Sequence[float],
                                       subtrahend: float
                                       ) -> Sequence[float]:
...
def _two_two_add(left_tail: float,
                 left_head: float,
                 right_tail: float,
                 right_head: float) -> Tuple[float, float, float, float]:
...
def _cross_product(first_dx: float,
                   first_dy: float,
                   second_dx: float,
                   second_dy: float) -> Tuple[float, float, float, float]:
...
def _scale_by_squared_length(components: Sequence[float],
                             dx: float,
                             dy: float) -> Sequence[float]:
...
def _add_extras(final_components: Sequence[float],
                first_dx: float,
                first_dx_tail: float,
                first_dy: float,
                first_dy_tail: float,
...
def _adaptive_vectors_cross_product(first_start_x: float,
    ...
    threshold = first_upper_bound_coefficient * upper_bound
    if (estimation >= threshold) or (-estimation >= threshold):
        return list(dropwhile(not_, first_components))
    minuend_x_tail = _two_subtract_tail(first_end_x, first_start_x,
                                        minuend_x)
    subtrahend_x_tail = _two_subtract_tail(second_end_x, second_start_x,
                                           subtrahend_x)
